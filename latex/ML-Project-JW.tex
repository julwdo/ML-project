...
The \href{https://archive.ics.uci.edu/dataset/848/secondary+mushroom+dataset}{dataset} used in this study is a simulated version inspired by the \href{https://archive.ics.uci.edu/dataset/73/mushroom}{Mushroom Data Set}. It contains 61,069 hypothetical mushrooms, each described by 20 features and classified as either definitely edible or definitely poisonous/of unknown edibility. The class distribution is balanced, with 27,181 (44.51\%) mushrooms classified as edible and 33,888 (55.49\%) as poisonous, ensuring that no class is overrepresented in subsequent analyses. During preprocessing, 146 (0.24\%) duplicate rows were identified and removed to prevent undue weighting of any observation. This step reduced the dataset to 60,923 mushrooms, of which 27,181 (44.62\%) edible and 33,742 (55.38\%) poisonous. Furthermore, nine variables were identified as containing missing values, as shown in Table \ref{tab:missing_values_before}.

Looking at the possible values of the variables, some values not defined by the author of the dataset were found, e.g., value ``d'' for cap-surface or value ``f'' for stem-root. An overview over all 21 variables, their types and possible values is presented in Table \ref{tab:mushroom_variables}.

One can see that for many variables, one of the possible values is ``f'' which stands either for ``no'' or ``none''. A question arose about whether the value ``f'' should be treated as a regular value or as missing. After careful consideration, it was decided to encode all occurrences of ``f'' as missing, except in ``does-bruise-or-bleed'' and ``has-ring'', where ``f'' should be interpreted as ``no'' rather than ``none''. Then also some statistics on the numerical variables were explored. A minimum of 0 for ``stem-height'' and ``stem-width'' revealed the presence of 915 mushrooms with ``stem-height'' equal to 0 and ``stem-width''. These turned out to be the same mushrooms, meaning that if ``stem-height'' was equal to 0, also ``stem-width'' was equal to 0. Hence, also the other variables regarding stem characteristics where explored. It turned out that for these 915 mushrooms, all stem-root, stem-surface and stem-color were equal to f. Hence, ``stem-height'' and ``stem-width'' equal to 0 were also considered as missing values. These operations increased the number of variables with missing values to fourteen (see Table \ref{tab:missing_values_after}).

However, since the implemented DecisionTree class is able to handle missing values, first analysis was performed with them in place. A nested cross-validation was performed with the following parameter grid:
...
Mean Test Error: 0.0601
Minimum Test Error: 0.0532
Best Model Parameters (corresponding to Minimum Test Error): {'min_samples_split': 2, 'max_depth': 15, 'n_features': 'log2', 'criterion': 'gini', 'min_information_gain': 0.0, 'n_quantiles': 5, 'isolate_one': False}
% Update these results!

Subsequently, columns with more than 45\% missing values were dropped and the remaining rows with missing values were dropped. Then, the nested cross validation was performed again, with the same parameter grid, and the following results were obtained. 
...
\footnotesize
\setlength{\tabcolsep}{5pt}
\caption{Missing Values Count and Percentage (After Replacements)}
...
stem-height & 915 & 1.50 \\ \hline
stem-width & 915 & 1.50 \\ \hline
gill-spacing & 52451 & 85.89 \\ \hline
